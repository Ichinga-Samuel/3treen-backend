...
module.exports = {
    ...
    makeReview:catchAsync(async(req,res,next)=>{
        ...
        var checkUser =await productModel.find({_id:req.params.id}).populate({ path: 'reviews', populate: 'reviewer'})
        var checkUserResult = checkUser[0].reviews.find(function(reviews33){return reviews33.reviewer._id == id})
        ...
        if(checkUserResult.reviewer._id != id ){
           ...
           return next(new AppError("hello, you cannot make two reviews on one product", 402))
        }
